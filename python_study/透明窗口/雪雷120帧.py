...
class Snowflake:
    def __init__(self):
        ...
        self.acceleration = random.uniform(0.0005, 0.003)
...
class Lightning:
    ...
    def generate_lightning_points(self):
        ...
        while current_y < Height:
            new_x = points[-1][0] + random.randint(-60, 60)
            new_y = current_y + random.randint(40, 120)
...
def main():
    ...
    running = True
    snowflakes = [Snowflake() for _ in range(400)]
    lightning = None
    lightning_timer = 0
    ...
    while running:
        ...
        if lightning:
            ...
            if random.random() < 0.003:  # 0.3% 的概率产生新的闪电
                lightning = Lightning()
                lightning_timer = lightning.lifetime

        pygame.display.update()
        clock.tick(120)  # 设置为120帧
