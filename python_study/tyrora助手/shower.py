...
class Stats:
    def __init__(self):
        self.ui = QUiLoader().load(os.path.join(当前工作目录,'ui/untitled.ui'))

        self.ui.pushButton.clicked.connect(self.去图片夹冗余文件)            
        self.ui.pushButton_2.clicked.connect(self.去图片冗余文件)
        self.ui.pushButton_3.clicked.connect(self.检查文件夹内md文件的图片完整性)
        self.ui.pushButton_4.clicked.connect(self.检查文md文件的图片完整性)
        self.ui.pushButton_5.clicked.connect(self.md和assets和md内图片全部换名字)
